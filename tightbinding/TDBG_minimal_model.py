...
def build_hamiltonian(kx, ky):
    ...
    for i in range(n_sites):
        ...
        p1 = rotate(k_vec + G - K_1, 0 * theta_rad/2)
        p2 = rotate(k_vec + G - K_2, -0 * theta_rad/2)
        ...
        g1 = g_mat(p1[0], p1[1])
        H[idx1:idx1 + 2, idx2:idx2 + 2] = g1.conj().T
        H[idx2:idx2 + 2, idx1:idx1 + 2] = g1
        ...
        g2 = g_mat(p2[0], p2[1])
        H[idx3:idx3 + 2, idx4:idx4 + 2] = g2.conj().T
        H[idx4:idx4 + 2, idx3:idx3 + 2] = g2

        # ==== 层间莫尔耦合 U (仅耦合相邻的 Layer 2 与 Layer 3) ====
        # T1 通道 (m, n) -> (m, n)
        H[idx2:idx2 + 2, idx3:idx3 + 2] = T1.conj().T

        H[idx3:idx3 + 2, idx2:idx2 + 2] = T1
        # T2 通道 (m, n) -> (m + valley, n)
        if (m + 1 * valley, n) in invL:
            j2 = invL[(m + 1 * valley, n)]
            idx3_j2 = 8 * j2 + 4
            H[idx2:idx2 + 2, idx3_j2:idx3_j2 + 2] = T2.conj().T
            H[idx3_j2:idx3_j2 + 2, idx2:idx2 + 2] = T2
